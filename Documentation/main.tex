...
A fordítási folyamat lépéseit a \ref{fig:pipeline_forditas}. ábra szemlélteti.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pipeline_transpile}
...
A főbb grammatikai szabályok railroad diagramjai a \ref{fig:rr_query}--\ref{fig:rr_caseexpr}. ábrákon láthatók.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{selectQuery}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{selectStmt}
    \caption{A \texttt{selectStmt} szabály railroad diagramja}
    \label{fig:rr_selectstmt}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{condition}
    \caption{A \texttt{condition} szabály railroad diagramja}
...
\begin{figure}[H]
    ...
    \caption{Az \texttt{expr} szabály railroad diagramja}
    \label{fig:rr_expr}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{caseExpr}
    \caption{A \texttt{caseExpr} szabály railroad diagramja}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{deleteStmt}
    \caption{A \texttt{deleteStmt} szabály railroad diagramja}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{insertStmt}
    \caption{Az \texttt{insertStmt} szabály railroad diagramja}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{updateStmt}
    \caption{Az \texttt{updateStmt} szabály railroad diagramja}
...
A fordító helyességének igazolása három egymást kiegészítő tesztelési szinten történik. Mindhárom szint más-más típusú hibát képes felderíteni, és együttesen lefedik a fordítás teljes folyamatát a szintaktikai elemzéstől a szemantikai egyenértékűségig. A tesztelési infrastruktúra felépítését a \ref{fig:pipeline_teszteles}. ábra mutatja.

\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{pipeline_testing}
    \caption{A tesztelési pipeline felépítése}
...
A projekt egyik motivációja az volt, hogy a nagy nyelvi modellek (LLM) ugyan képesek SQL lekérdezéseket LINQ kóddá alakítani, de ez a folyamat statisztikai alapú, nem determinisztikus, és nem integrálható automatizált pipeline-ba. Ennek empirikus vizsgálatára méréseket végeztem, amelyek során két kereskedelmi LLM-et hasonlítottam össze a saját fordítóval ugyanazon a bemeneten.

\subsection{Módszertan}

A mérés bemenete a projekt stressz tesztelésére összeállított komplex SQL lekérdezés, amely egyszerre tartalmaz háromtáblás JOIN-t, beágyazott \texttt{CASE WHEN} kifejezést, korrelált és nem korrelált allekérdezéseket, \texttt{EXISTS} és \texttt{NOT IN} feltételeket, szöveg- és dátumfüggvényeket, \texttt{COALESCE}-t, \texttt{CAST}-ot, valamint \texttt{ORDER BY}, \texttt{LIMIT} és \texttt{OFFSET} záradékokat. A lekérdezés szándékosan összetett, hogy megvizsgálja, hogyan birkóznak meg az LLM-ek a grammatikailag és szemantikailag bonyolult bemenetekkel.

Minden modell ugyanazt a promptot kapta, amely kizárólag a LINQ kifejezést kérte vissza, magyarázat és formázás nélkül. A mérést mindkét modellnél 5 alkalommal végeztem el, és rögzítettem a válaszidőt, a kimenet tartalmát, valamint azt, hogy a különböző futtatások azonos kimenetet adtak-e.

A két vizsgált LLM a Gemini 3.6 Flash (Google) és a GPT-OSS-120B (OpenAI, Groq infrastruktúrán futtatva) volt. A saját fordító mérései egy fejlesztői gépen, lokálisan futtatva történtek, ahol az első hívás utáni JIT warm-up időt nem vettük figyelembe. Fontos megjegyezni, hogy a válaszidőn kívül az erőforrásfelhasználásban is nagyságrendi különbségek állnak fenn: az LLM-ek esetén nagyteljesítményű felhőinfrastruktúra fut a háttérben, míg a saját fordító egy átlagos fejlesztői gépen is ezredmásodpercek alatt elvégzi a feladatot.

Az összehasonlítás alapját a generált kimenetek karakterszintű összevetése adja, kiegészítve strukturális elemzéssel. Karakterhelyes egyezés az LLM-ek esetén nem várható, mivel a modellek ugyanarra a bemenetre különböző, de esetleg szemantikailag egyenértékű kódot generálhatnak. Ezért a konzisztenciát nem karakterszintű azonosságként értelmezzük, hanem azt vizsgáljuk, hogy a különböző futtatások strukturálisan hasonló vagy eltérő megoldásokat adnak-e.

\subsection{Eredmények}

\begin{table}[h]
\centering
\caption{LLM összehasonlítás eredményei (5 futtatás)}
\label{tab:llm-comparison}
\renewcommand{\arraystretch}{1.4}
\small
\begin{tabular}{|l|r|r|r|r|c|}
\hline
\textbf{Modell} & \textbf{Átlag} & \textbf{Min} & \textbf{Max} & \textbf{Szórás} & \textbf{Konzisztens} \\
\hline
Gemini 3.6 Flash      & 46,18 s & 17,43 s & 71,30 s & 22,1 s & nem \\
GPT-OSS-120B (Groq)   & 13,82 s &  3,43 s & 20,02 s &  6,8 s & nem \\
Saját fordító (lokális) & $<$1 ms &  $<$1 ms &  $<$1 ms & $\approx$0 & igen \\
\hline
\end{tabular}
\end{table}

A Gemini esetén a válaszidők szórása 22,1 másodperc, ami a minimális és maximális értékek közötti közel négyszeres különbséget tükröz. A Groq modellnél a szórás 6,8 másodperc, a legrövidebb és leghosszabb válasz között közel hatszoros eltérés mutatkozik. Ez a szórás önmagában is jelzi, hogy az LLM-ek válaszideje kiszámíthatatlan, ami automatizált pipeline-ban tervezési nehézséget jelent.

\subsection{Megfigyelések}

A válaszidő tekintetében a különbség több nagyságrendnyi. A Gemini átlagosan 46 másodpercet vett igénybe ugyanahhoz a lekérdezéshez, amelyet a saját fordító ezredmásodpercek alatt dolgoz fel lokálisan. A Groq infrastruktúrán futó GPT-OSS-120B gyorsabbnak bizonyult, de a 13 másodperces átlag és a 20 másodperces maximum még mindig lényegesen több, mint ami egy automatizált pipeline-ban elfogadható lenne.

A konzisztencia mindkét LLM esetén hiányzott. A Gemini 5 futtatásán 5 strukturálisan különböző kimenetet produkált. A Groq modell szintén eltérő megoldásokat adott futtatásonként, emellett két futtatás üres választ, egy pedig csonka, félbeszakadt kimenetet adott vissza. A saját fordító mind az 5 futtatáson byte-szintűen azonos kimenetet állított elő.

A kimenet helyességét vizsgálva szembetűnő strukturális hibák is megfigyelhetők. A Groq modell első válaszában a \texttt{WHERE} feltétel a \texttt{JOIN} műveletek elé kerül, ami az EF Core lekérdezési láncban szemantikailag helytelen sorrendet jelent. A \texttt{DbContext} elnevezése futtatásonként változott (\texttt{ctx}, \texttt{context}, \texttt{db}), és a \texttt{LIKE 'B\%'} operátor egyszer \texttt{EF.Functions.Like}-ra, másszor \texttt{StartsWith()}-re fordult. A saját fordító mindkét esetben provider-független, EF Core kompatibilis, determinisztikus kimenetet állít elő.

\subsection{Következtetések}

Az LLM-ek interaktív segítségként hasznos eszközök lehetnek, ha egy fejlesztő egyszeri alkalommal szeretne egy SQL lekérdezést LINQ-ra fordítani és az eredményt manuálisan ellenőrzi. Automatizált migrációs pipeline-ban azonban, ahol több száz vagy több ezer lekérdezést kell feldolgozni, a válaszidő, a nem determinisztikus kimenet, az ellenőrizhetőség hiánya és a felhőinfrastruktúra szükségessége komoly akadályt jelent. A determinisztikus fordító ezzel szemben programozottan hívható, az eredménye verifikálható, lokálisan futtatható, és ugyanarra a bemenetre mindig ugyanazt adja.
